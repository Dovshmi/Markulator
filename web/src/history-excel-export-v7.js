...
const asNumber = (v) => { const n = Number(v); return Number.isFinite(n) ? n : ''; };
...
function blank(span, height = 10) { return merge('', span, 'Blank', `ss:Height="${height}"`); }
...
function plusMmFormula() {
  const plusRound = 'ROUND(C9*$B$13,2)';
  const nomRound = 'ROUND(B10*$B$13,2)';
  const over = `ROUND(${nomRound}+${plusRound},2)>B10*$B$13+C9*$B$13`;
  return `=IF(${over},IF(${plusRound}>0,${plusRound}-0.01,${plusRound}),${plusRound})`;
}
function nomMmFormula() {
  const plusRound = 'ROUND(C9*$B$13,2)';
  const nomRound = 'ROUND(B10*$B$13,2)';
  const minusRound = 'ROUND(C11*$B$13,2)';
  const over = `ROUND(${nomRound}+${plusRound},2)>B10*$B$13+C9*$B$13`;
  const under = `ROUND(${nomRound}-${minusRound},2)<B10*$B$13-C11*$B$13`;
  return `=${nomRound}+IF(${over},IF(${plusRound}>0,0,-0.01),0)+IF(${under},IF(${minusRound}>0,0,0.01),0)`;
}
function minusMmFormula() {
  const nomRound = 'ROUND(B10*$B$13,2)';
  const minusRound = 'ROUND(C11*$B$13,2)';
  const under = `ROUND(${nomRound}-${minusRound},2)<B10*$B$13-C11*$B$13`;
  return `=IF(${under},IF(${minusRound}>0,${minusRound}-0.01,${minusRound}),${minusRound})`;
}
function maxMmFormula() {
  const rounded = 'ROUND(B5*$B$8,2)';
  return `=IF(${rounded}>B5*$B$8,${rounded}-0.01,${rounded})`;
}
function minMmFormula() {
  const rounded = 'ROUND(B6*$B$8,2)';
  return `=IF(${rounded}<B6*$B$8,${rounded}+0.01,${rounded})`;
}
...
function menuSheet(tolerance, maxMin) {
  return worksheet('Menu', [220, 110, 520], [
    ...
    merge('Click OPEN SHEET in a history row. Each saved calculation has its own calculator sheet.', 3, 'Note', 'ss:Height="36"'),
    blank(3),
    header(['Section', 'Rows', 'Open']),
    row([cell('Tolerance ±', 'String', 'Option'), cell(tolerance.length, 'Number', 'ResultBig'), linkCell('Open Tolerance History', 'Tolerance_History')], 'ss:Height="26"'),
    row([cell('Maximum / Minimum', 'String', 'Option'), cell(maxMin.length, 'Number', 'ResultBig'), linkCell('Open Max-Min History', 'Max_Min_History')], 'ss:Height="26"'),
  ]);
}
...
function toleranceHistorySheet(items) {
  ...
  items.forEach((item, index) => {
    ...
    const sheet = `TOL_${index + 1}`;
    rows.push(row([
      linkCell('OPEN SHEET', sheet), cell(savedAt(item)), cell(d), cell(d === 'inch → mm' ? 'in' : 'mm'), cell(d === 'inch → mm' ? 'mm' : 'in'),
      cell(asNumber(tol.positive), 'Number', 'Input'), cell(asNumber(tol.nominal), 'Number', 'Input'), cell(asNumber(tol.negative), 'Number', 'Input'),
      linkCell(sheet, sheet), linkCell('view', sheet), linkCell('view', sheet), linkCell('view', sheet),
    ], 'ss:Height="25"'));
  });
  return worksheet('Tolerance_History', [120, 120, 100, 75, 82, 85, 90, 85, 100, 85, 90, 85], rows);
}
...
function maxHistorySheet(items) {
  ...
  items.forEach((item, index) => {
    ...
    const d = direction(item);
    const sheet = `MAX_${index + 1}`;
    rows.push(row([
      linkCell('OPEN SHEET', sheet), cell(savedAt(item)), cell(d), cell(d === 'inch → mm' ? 'in' : 'mm'), cell(d === 'inch → mm' ? 'mm' : 'in'),
      cell(asNumber(limits.max), 'Number', 'Input'), cell(asNumber(limits.min), 'Number', 'Input'),
      linkCell(sheet, sheet), linkCell('view', sheet), linkCell('view', sheet),
    ], 'ss:Height="25"'));
  });
  return worksheet('Max_Min_History', [120, 120, 100, 75, 82, 85, 85, 100, 85, 85], rows);
}
...
function toleranceCalcSheet(item, index) {
  ...
  const rows = [
    ...
    row([linkCell('← Back to Tolerance History', 'Tolerance_History', 'BackLink', ' ss:MergeAcross="13"')], 'ss:Height="22"'),
    blank(14, 16),
    ...
    row([cell('Max', 'String', 'GiantLabel'), formula('=B10+C9', 'LargeFormula'), formula('=F10+G9', 'LargeFormula'), cell('', 'String'), cell('', 'String'), cell('', 'String', 'Box', ' ss:MergeAcross="1"'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String')], 'ss:Height="38"'),
    row([cell('Min', 'String', 'GiantLabel'), formula('=B10-C11', 'LargeFormula'), formula('=F10-G11', 'LargeFormula'), cell('', 'String'), cell('', 'String'), cell('', 'String', 'Box', ' ss:MergeAcross="1"'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String')], 'ss:Height="38"'),
    redBar(14),
    ...
    row([cell('', 'String'), cell('+', 'String', 'Sign'), inToMm ? cell(asNumber(tol.positive), 'Number', 'Input') : formula('=G9/$B$13', 'Formula'), cell('', 'String'), cell('', 'String'), cell('+', 'String', 'Sign'), inToMm ? formula(plusMmFormula(), 'Formula') : cell(asNumber(tol.positive), 'Number', 'Input'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String'), formula('=G9', 'Formula'), formula('=G9', 'Formula'), formula('=F10+G9', 'Formula')], 'ss:Height="38"'),
    row([cell('Nom', 'String', 'GiantLabel'), inToMm ? cell(asNumber(tol.nominal), 'Number', 'InputLarge') : formula('=F10/$B$13', 'LargeFormula'), cell('', 'String'), cell('', 'String'), cell('Nom', 'String', 'GiantLabel'), inToMm ? formula(nomMmFormula(), 'ResultLarge') : cell(asNumber(tol.nominal), 'Number', 'InputLarge'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String'), formula('=F10', 'Formula'), cell('', 'String'), cell('', 'String'), cell('', 'String')], 'ss:Height="42"'),
    row([cell('', 'String'), cell('-', 'String', 'Sign'), inToMm ? cell(asNumber(tol.negative), 'Number', 'Input') : formula('=G11/$B$13', 'Formula'), cell('', 'String'), cell('', 'String'), cell('-', 'String', 'Sign'), inToMm ? formula(minusMmFormula(), 'Formula') : cell(asNumber(tol.negative), 'Number', 'Input'), cell('', 'String'), cell('', 'String'), cell('', 'String'), cell('', 'String'), formula('=G11', 'Formula'), formula('=G11', 'Formula'), formula('=F10-G11', 'Formula')], 'ss:Height="38"'),
    blank(14, 12),
...
function maxCalcSheet(item, index) {
  ...
  const rows = [
    ...
    row([linkCell('← Back to Max-Min History', 'Max_Min_History', 'BackLink', ' ss:MergeAcross="9"')], 'ss:Height="22"'),
    blank(10, 16),
    ...
    row([cell('Max', 'String', 'GiantLabel'), inToMm ? cell(asNumber(limits.max), 'Number', 'InputLarge') : formula('=C5/$B$8', 'LargeFormula'), inToMm ? formula(maxMmFormula(), 'ResultLarge') : cell(asNumber(limits.max), 'Number', 'InputLarge'), cell('', 'String'), cell('', 'String'), cell('', 'String', 'Box', ' ss:MergeAcross="1"'), cell('', 'String'), cell('', 'String'), cell('', 'String')], 'ss:Height="42"'),
    row([cell('Min', 'String', 'GiantLabel'), inToMm ? cell(asNumber(limits.min), 'Number', 'InputLarge') : formula('=C6/$B$8', 'LargeFormula'), inToMm ? formula(minMmFormula(), 'ResultLarge') : cell(asNumber(limits.min), 'Number', 'InputLarge'), cell('', 'String'), cell('', 'String'), cell('', 'String', 'Box', ' ss:MergeAcross="1"'), cell('', 'String'), cell('', 'String'), cell('', 'String')], 'ss:Height="42"'),
    redBar(10),
    row([cell('Factor', 'String', 'SmallLabel'), cell(MM, 'Number', 'Input'), cell('mm / inch', 'String', 'Unit'), cell('', 'String'), cell('Range', 'String', 'SmallLabel'), formula('=C5-C6', 'ResultBig'), cell('mm', 'String', 'Unit'), cell('', 'String'), cell('', 'String'), cell('', 'String')], 'ss:Height="30"'),
    row([cell('Range', 'String', 'SmallLabel'), formula('=B5-B6', 'ResultBig'), cell('in', 'String', 'Unit'), cell('', 'String'), cell('Logic', 'String', 'SmallLabel'), cell('Markulator safe rounding', 'String', 'Callout', ' ss:MergeAcross="1"')], 'ss:Height="30"'),
    redBar(10),
