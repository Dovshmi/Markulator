...
function formula(formulaText, style = 'Formula', extra = '', dataType = 'Number') { return `<Cell ss:StyleID="${style}" ss:Formula="${xml(formulaText)}"${extra}><Data ss:Type="${dataType}">0</Data></Cell>`; }
function textFormula(formulaText, style = 'FormulaText', extra = '') { return `<Cell ss:StyleID="${style}" ss:Formula="${xml(formulaText)}"${extra}><Data ss:Type="String"></Data></Cell>`; }
...
function noteRow(text, span) { return row([cell(text, 'String', 'Note', ` ss:MergeAcross="${span - 1}"`)], 'ss:Height="38"'); }
...
function toleranceSummary(item, index, r) {
  ...
  return row([
    cell(`＋ ${index + 1}`, 'String', 'OpenIndex'), cell(savedAt(item)), cell('Tolerance ±'), cell(direction(item), 'String', 'InputText'),
    ...
    cell('לחץ על + בשוליים / use left +', 'String', 'ShowHint'),
  ], 'ss:Collapsed="1" ss:Height="25"');
}
function toleranceCard(index, r) {
  return [
    ...
    row([cell('MAX', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C8+R${r}C7`, 'BigInput', ' ss:MergeAcross="3"'), textFormula(`=R${r}C5`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('MAX', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C13`, 'BigResult', ' ss:MergeAcross="3"'), textFormula(`=R${r}C6`, 'UnitPill', ' ss:MergeAcross="1"')], `${hidden()} ss:Height="34"`),
    row([cell('NOM', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C8`, 'BigInput', ' ss:MergeAcross="3"'), textFormula(`=R${r}C5`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('NOM', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C11`, 'BigResult', ' ss:MergeAcross="3"'), textFormula(`=R${r}C6`, 'UnitPill', ' ss:MergeAcross="1"')], `${hidden()} ss:Height="34"`),
    row([cell('MIN', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C8-R${r}C9`, 'BigInput', ' ss:MergeAcross="3"'), textFormula(`=R${r}C5`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('MIN', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C14`, 'BigResult', ' ss:MergeAcross="3"'), textFormula(`=R${r}C6`, 'UnitPill', ' ss:MergeAcross="1"')], `${hidden()} ss:Height="34"`),
...
function maxSummary(item, index, r) {
  ...
  return row([
    cell(`＋ ${index + 1}`, 'String', 'OpenIndex'), cell(savedAt(item)), cell(direction(item), 'String', 'InputText'),
...
function maxCard(index, r) {
  return [
    ...
    row([cell('MAX', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C6`, 'BigInput', ' ss:MergeAcross="1"'), textFormula(`=R${r}C4`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('MAX', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C8`, 'BigResult', ' ss:MergeAcross="1"'), textFormula(`=R${r}C5`, 'UnitPill', ' ss:MergeAcross="1"')], `${hidden()} ss:Height="34"`),
    row([cell('MIN', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C7`, 'BigInput', ' ss:MergeAcross="1"'), textFormula(`=R${r}C4`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('MIN', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C9`, 'BigResult', ' ss:MergeAcross="1"'), textFormula(`=R${r}C5`, 'UnitPill', ' ss:MergeAcross="1"')], `${hidden()} ss:Height="34"`),
    row([cell('RANGE', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C6-R${r}C7`, 'BigInput', ' ss:MergeAcross="1"'), textFormula(`=R${r}C4`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('RANGE', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${r}C10`, 'BigResult', ' ss:MergeAcross="1"'), textFormula(`=R${r}C5`, 'UnitPill', ' ss:MergeAcross="1"')], `${hidden()} ss:Height="34"`),
...
function sheet(name, columns, rows, split = 4) {
  return `<Worksheet ss:Name="${xml(name)}"><Table>${columns.map((w) => `<Column ss:Width="${w}"/>`).join('')}${rows.join('')}</Table><WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel"><FreezePanes/><FrozenNoSplit/><SplitHorizontal>${split}</SplitHorizontal><TopRowBottomPane>${split}</TopRowBottomPane><DoNotDisplayGridlines/><OutlineSummaryBelow>False</OutlineSummaryBelow></WorksheetOptions></Worksheet>`;
}
function buildTolerance(items) {
  const rows = [titleRow('Tolerance ± History', 15), noteRow('Every visible row is one saved calculation. Use the + in the far-left Excel outline area to open the calculator block below the row. The first column also marks each expandable row with ＋.', 15), '<Row/>', headerRow(['＋ / #', 'Saved At', 'Mode', 'Direction', 'In Unit', 'Out Unit', 'Tol+ In', 'Nom In', 'Tol- In', 'Tol+ Out', 'Nom Out', 'Tol- Out', 'MAX Out', 'MIN Out', 'How to open'])];
  let rNum = 5;
  ...
  return sheet('Tolerance', [58, 118, 92, 96, 70, 76, 82, 92, 82, 90, 100, 90, 94, 94, 175], rows);
}
function buildMaxMin(items) {
  const rows = [titleRow('Maximum / Minimum History', 11), noteRow('Every visible row is one saved calculation. Use the + in the far-left Excel outline area to open the calculator block below the row. The first column also marks each expandable row with ＋.', 11), '<Row/>', headerRow(['＋ / #', 'Saved At', 'Direction', 'In Unit', 'Out Unit', 'MAX In', 'MIN In', 'MAX Out', 'MIN Out', 'Range', 'How to open'])];
  let rNum = 5;
  ...
  return sheet('Max-Min', [58, 118, 98, 70, 76, 92, 92, 98, 98, 90, 175], rows);
}
function buildMenu(toleranceCount, maxCount) {
  return sheet('Menu', [230, 130, 430], [titleRow('Markulator Excel Calculator Export', 3), noteRow('Open a calculator sheet. Each sheet is a history list. To show a calculation, click the + outline control in the far-left Excel margin beside the row.', 3), '<Row/>', headerRow(['Calculator sheet', 'Saved rows', 'Description']), row([cell('Tolerance ±', 'String', 'Option'), cell(toleranceCount, 'Number', 'BigResult'), cell('History list with expandable MAX / NOM / MIN calculator cards')]), row([cell('Maximum / Minimum', 'String', 'Option'), cell(maxCount, 'Number', 'BigResult'), cell('History list with expandable MAX / MIN / RANGE calculator cards')])], 4);
}
