...
function toleranceSummary(item, index, r) {
  ...
  return row([
    cell(index + 1, 'Number', 'Index'), cell(dateText(item)), cell('Tolerance ±'), cell(direction(item), 'String', 'InputText'),
    ...
    cell('All calculation cards open together below', 'String', 'ShowHint'),
  ], 'ss:Height="25"');
}
function toleranceCard(index, summaryRow) {
  const h = hiddenAttrs();
  return [
    ...
    row([cell('MAX', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C8+R${summaryRow}C7`, 'BigInput', ' ss:MergeAcross="3"'), textFormula(`=R${summaryRow}C5`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('MAX', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C13`, 'BigResult', ' ss:MergeAcross="3"'), textFormula(`=R${summaryRow}C6`, 'UnitPill', ' ss:MergeAcross="1"')], `${h} ss:Height="36"`),
    row([cell('NOM', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C8`, 'BigInput', ' ss:MergeAcross="3"'), textFormula(`=R${summaryRow}C5`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('NOM', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C11`, 'BigResult', ' ss:MergeAcross="3"'), textFormula(`=R${summaryRow}C6`, 'UnitPill', ' ss:MergeAcross="1"')], `${h} ss:Height="36"`),
    row([cell('MIN', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C8-R${summaryRow}C9`, 'BigInput', ' ss:MergeAcross="3"'), textFormula(`=R${summaryRow}C5`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('MIN', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C14`, 'BigResult', ' ss:MergeAcross="3"'), textFormula(`=R${summaryRow}C6`, 'UnitPill', ' ss:MergeAcross="1"')], `${h} ss:Height="36"`),
    row([cell('Tol +', 'String', 'SmallLabel'), formula(`=R${summaryRow}C7`, 'Input'), cell('Tol -', 'String', 'SmallLabel'), formula(`=R${summaryRow}C9`, 'Input'), cell('Exact conversion uses 25.4', 'String', 'Callout', ' ss:MergeAcross="2"'), cell('', 'String', 'RedDivider'), cell('Tol+ result', 'String', 'SmallLabel'), formula(`=R${summaryRow}C10`, 'Formula'), cell('Tol- result', 'String', 'SmallLabel'), formula(`=R${summaryRow}C12`, 'Formula'), cell('Safe rounding', 'String', 'Callout', ' ss:MergeAcross="1"')], `${h} ss:Height="30"`),
    blank(15, h),
  ];
}
function maxSummary(item, index, r) {
  ...
  return row([
    cell(index + 1, 'Number', 'Index'), cell(dateText(item)), cell(direction(item), 'String', 'InputText'),
...
function maxCard(index, summaryRow) {
  const h = hiddenAttrs();
  return [
    ...
    row([cell('MAX', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C6`, 'BigInput', ' ss:MergeAcross="1"'), textFormula(`=R${summaryRow}C4`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('MAX', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C8`, 'BigResult', ' ss:MergeAcross="1"'), textFormula(`=R${summaryRow}C5`, 'UnitPill', ' ss:MergeAcross="1"')], `${h} ss:Height="36"`),
    row([cell('MIN', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C7`, 'BigInput', ' ss:MergeAcross="1"'), textFormula(`=R${summaryRow}C4`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('MIN', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C9`, 'BigResult', ' ss:MergeAcross="1"'), textFormula(`=R${summaryRow}C5`, 'UnitPill', ' ss:MergeAcross="1"')], `${h} ss:Height="36"`),
    row([cell('RANGE', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C6-R${summaryRow}C7`, 'BigInput', ' ss:MergeAcross="1"'), textFormula(`=R${summaryRow}C4`, 'UnitPill', ' ss:MergeAcross="1"'), cell('', 'String', 'RedDivider'), cell('RANGE', 'String', 'BigLabel', ' ss:MergeAcross="1"'), formula(`=R${summaryRow}C10`, 'BigResult', ' ss:MergeAcross="1"'), textFormula(`=R${summaryRow}C5`, 'UnitPill', ' ss:MergeAcross="1"')], `${h} ss:Height="36"`),
    row([cell('Exact max', 'String', 'SmallLabel'), formula(`=IF(R${summaryRow}C3="inch → mm",R${summaryRow}C6*${MM_PER_INCH},R${summaryRow}C6/${MM_PER_INCH})`, 'CardFormula'), cell('Exact min', 'String', 'SmallLabel'), formula(`=IF(R${summaryRow}C3="inch → mm",R${summaryRow}C7*${MM_PER_INCH},R${summaryRow}C7/${MM_PER_INCH})`, 'CardFormula'), cell('', 'String', 'RedDivider'), cell('Safe rounding follows Markulator', 'String', 'Callout', ' ss:MergeAcross="4"')], `${h} ss:Height="30"`),
    blank(11, h),
  ];
}
...
function toleranceSheet(items) {
  const rows = [merged('Tolerance ± History', 15, 'Title', 'ss:Height="34"'), merged('History list first. To show all calculator cards together, click the outline level 2 button at the top-left of the row area, or use Data → Show Detail on the SHOW ALL row.', 15, 'Note', 'ss:Height="44"'), '<Row/>', header(['#', 'Saved At', 'Mode', 'Direction', 'In Unit', 'Out Unit', 'Tol+ In', 'Nom In', 'Tol- In', 'Tol+ Out', 'Nom Out', 'Tol- Out', 'MAX Out', 'MIN Out', 'Note'])];
  const summaryStart = 5;
  if (!items.length) rows.push(merged('No saved Tolerance ± calculations were available.', 15, 'Note'));
  items.forEach((item, index) => rows.push(toleranceSummary(item, index, summaryStart + index)));
  rows.push(merged('SHOW ALL CALCULATIONS BELOW — click Excel outline level 2 / Data → Show Detail', 15, 'ShowAll', 'ss:Height="32"'));
  items.forEach((_, index) => rows.push(...toleranceCard(index, summaryStart + index)));
  return worksheet('Tolerance', [48, 118, 92, 96, 70, 76, 82, 92, 82, 90, 100, 90, 94, 94, 230], rows);
}
function maxMinSheet(items) {
  const rows = [merged('Maximum / Minimum History', 11, 'Title', 'ss:Height="34"'), merged('History list first. To show all calculator cards together, click the outline level 2 button at the top-left of the row area, or use Data → Show Detail on the SHOW ALL row.', 11, 'Note', 'ss:Height="44"'), '<Row/>', header(['#', 'Saved At', 'Direction', 'In Unit', 'Out Unit', 'MAX In', 'MIN In', 'MAX Out', 'MIN Out', 'Range', 'Note'])];
  const summaryStart = 5;
  if (!items.length) rows.push(merged('No saved Maximum / Minimum calculations were available.', 11, 'Note'));
  items.forEach((item, index) => rows.push(maxSummary(item, index, summaryStart + index)));
  rows.push(merged('SHOW ALL CALCULATIONS BELOW — click Excel outline level 2 / Data → Show Detail', 11, 'ShowAll', 'ss:Height="32"'));
  items.forEach((_, index) => rows.push(...maxCard(index, summaryStart + index)));
  return worksheet('Max-Min', [48, 118, 98, 70, 76, 92, 92, 98, 98, 90, 230], rows);
}
function menuSheet(tolCount, maxCount) {
  return worksheet('Menu', [230, 130, 510], [merged('Markulator Excel Calculator Export', 3, 'Title', 'ss:Height="34"'), merged('First click Enable Editing. Each calculator sheet has a history table first, then one SHOW ALL CALCULATIONS section. Open it once to reveal all calculator cards together.', 3, 'Note', 'ss:Height="44"'), '<Row/>', header(['Calculator sheet', 'Saved rows', 'Description']), row([cell('Tolerance ±', 'String', 'Option'), cell(tolCount, 'Number', 'BigResult'), cell('History table + one expandable section containing all MAX / NOM / MIN calculator cards')]), row([cell('Maximum / Minimum', 'String', 'Option'), cell(maxCount, 'Number', 'BigResult'), cell('History table + one expandable section containing all MAX / MIN / RANGE calculator cards')])], 4);
}
